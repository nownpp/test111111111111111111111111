...
export default function Home() {
  ...
  // التمرير التلقائي للأسفل عند توليد محتوى جديد
  useEffect(() => {
    if (step === 3 && !isEditingContent) {
      window.scrollTo({
        top: document.documentElement.scrollHeight,
        behavior: 'smooth'
      });
    }
  }, [generatedContent, step, isEditingContent]);
  ...
  const handleGenerateOutline = async (e: React.FormEvent) => {
    ...
    try {
      ...
      // تحديث الرصيد من الهيدر
      const newCreditsHeader = res.headers.get('X-New-Credits');
      if (newCreditsHeader) {
        const updatedSession = { ...currentUser, credits: Number(newCreditsHeader) };
        setCurrentUser(updatedSession);
        localStorage.setItem('bt_user', JSON.stringify(updatedSession));
      }
  ...
  const handleStartWriting = async () => {
    ...
    // إذا كنا في البداية تماماً
    if (currentStageIndex === 0 && !generatedContent) {
      setGeneratedContent(`## فهرس المحتويات\n\n${outlineText}\n\n---\n\n`);
    }
    ...
    try {
      ...
      if (!res.ok) {
        const data = await res.json();
        throw new Error(data.error || 'فشل الاتصال بالخادم أثناء كتابة المبحث.');
      }

      const reader = res.body?.getReader();
      const decoder = new TextDecoder();
      let done = false;
      let stageText = (stage.id !== 'intro' ? '\n\n' : '');
      const initialContent = generatedContent || `## فهرس المحتويات\n\n${outlineText}\n\n---\n\n`;
      ...
      while (!done && reader) {
        ...
        if (value) {
          const chunkValue = decoder.decode(value);
          stageText += chunkValue;
          setGeneratedContent(initialContent + stageText);
        }
      }
      ...
      setError(err.message || 'حدث خطأ أثناء الكتابة. يمكنك المحاولة مرة أخرى.');
    } finally {
      setIsLoading(false);
      setLoadingMessage('');
    }
            ...
            {step === 3 && (
              <div className="space-y-6 text-center">
                ...
                {isLoading ? (
                  ...
                  <div className="flex flex-col items-center justify-center py-6 gap-3">
                    <div className="bg-red-50 text-red-600 p-4 rounded-2xl text-sm border border-red-100 font-bold mb-2">
                      {error}
                    </div>
                    <button 
                      onClick={handleStartWriting}
                   ...
                   <div className="flex flex-col items-center justify-center py-6 gap-3">
                     <p className="text-gray-700 font-bold mb-2">تم الانتهاء من {currentStageIndex} من أصل 4 أجزاء</p>
                     
                     <div className="w-full bg-gray-200 rounded-full h-2.5 mb-4 max-w-[200px]">
                        <div className="bg-indigo-600 h-2.5 rounded-full transition-all duration-500" style={{ width: `${(currentStageIndex / 4) * 100}%` }}></div>
                     </div>

                     <button
                       onClick={handleStartWriting}
                       className="w-full bg-indigo-600 hover:bg-indigo-700 text-white font-bold py-4 rounded-xl transition-all shadow-lg flex items-center justify-center gap-2 active:scale-95"
                     >
                       <RefreshCw size={20} className={isLoading ? "animate-spin" : ""} />
                       استكمال كتابة الجزء التالي
                     </button>
                     
                     <p className="text-xs text-gray-500">ملاحظة: النقر هنا يرسل طلباً جديداً لضمان استقرار السيرفر.</p>
                   </div>
                ) : (
                  <div className="text-gray-400 py-8">بانتظار بدء المعالجة...</div>
                )}
              </div>
            )}
